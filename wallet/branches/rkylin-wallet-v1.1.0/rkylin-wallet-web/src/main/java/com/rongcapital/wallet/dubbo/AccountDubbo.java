...
public class AccountDubbo {
    ...
    public ResultVo openCompanyAccount(CompanyPo po) {
        ...
        logger.info("AccountDubbo.openCompanyAccount入参:{" + JsonUtil.bean2JsonStr(po) + "}");
...
//        accountBean.setRole(po.getRole());  // 角色号（场地方上送10） Y
        ...
        logger.info("AccountDubbo.openCompanyAccount dubbo入参:{" + JsonUtil.bean2JsonStr(accountBean) + "}");
        String response = acccountDubboService.saveMerchantAccount(accountBean);
        logger.info("AccountDubbo.openCompanyAccount dubbo反回:{" + response + "}");
        ResultVo vo = new ResultVo();
...
//        po.setCompanyName("北京融数沃雷特科技服务有限公司"); // 企业名称N
//        po.setBusLince("91110105MA0059308G"); // 营业执照 N
//        po.setUserId("201612091534060001"); //接入机构中设置的用户ID N
//        po.setUserType("1"); // 用户类型(1：商户 ) N
//        po.setConstId("M666666"); // 机构号 N
//        po.setProductId("P000221"); // 产品号 N
//        po.setUserName("万众企业账户");  // 用户名称，及接入机构的用户名 N (账户名称描述，随便写)
