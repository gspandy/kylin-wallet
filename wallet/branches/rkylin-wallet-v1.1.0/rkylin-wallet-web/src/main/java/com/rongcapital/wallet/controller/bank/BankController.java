...
public class BankController {
    ...
    public ActionResult bankDetail(ClientInfo clientInfo, String bankCardNum) throws Exception {
        ...
        if (null != bankInfo) {              
            if (!CardConstants.BANK_LIST.contains(new BankTypeVo(bankInfo.getBankCode()))) {
                result.setCode(ErrorCodeEnum.BANK_TYPE_ERROR.getCode());
                result.setMsg(ErrorCodeEnum.BANK_TYPE_ERROR.getValue());
            } else {
                result.setSuccess(true);
    ...
    public ActionResult bindCard(ClientInfo clientInfo, CardInfoVo cardInfo,String vcode) throws Exception {
        ActionResult result = new ActionResult();
        ...
        if (cardInfo == null || ValidateUtils.isEmpty(cardInfo.getBankCardNum())
                ...
                || ValidateUtils.isEmpty(cardInfo.getBankCode()) || ValidateUtils.isEmpty(cardInfo.getTel()) || ValidateUtils.isEmpty(vcode)) {
            result.setCode(ErrorCodeEnum.PARM_CHECK_NULL.getCode());
            result.setMsg(ErrorCodeEnum.PARM_CHECK_NULL.getValue());
        ...
        if (!CardConstants.BANK_LIST.contains(new BankTypeVo(cardInfo.getBankCode()))) {
            result.setCode(ErrorCodeEnum.BANK_TYPE_ERROR.getCode());
            result.setMsg(ErrorCodeEnum.BANK_TYPE_ERROR.getValue());
            logger.info("BankController.bindCard返回结果:{" + JsonUtil.bean2JsonStr(result) + "}");
            return result;
     ...
     // 判断验证码是否正确       
        SmsInfoVo data = SmsUtil.getData(cardInfo.getTel());
        ...
        // 删除验证码       
        SmsUtil.delToken(cardInfo.getTel());
        ...
        // 只能绑定自己的卡
        if (!user.getIdCard().equals(cardInfo.getIdCard()) || !user.getUserRealName().equals(cardInfo.getUserName())) {
    ...
    public ActionResult allBank(ClientInfo clientInfo) throws Exception {        
        ActionResult result = new ActionResult();  
        result.addResult("bankList",DateInitUtil.getBankList());
        result.setSuccess(true);     
        logger.info("BankController.allBank返回结果" + JsonUtil.bean2JsonStr(result));
        return result;
    }
