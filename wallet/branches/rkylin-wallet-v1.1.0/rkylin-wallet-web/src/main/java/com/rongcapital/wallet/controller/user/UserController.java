...
public class UserController {
    ...
    public ActionResult register(ClientInfo clientInfo, UserInfoVo userInfo, String sign) throws Exception {

        ActionResult result = new ActionResult();
        // 临时屏蔽注册
        if (true) {
            result.setCode(ErrorCodeEnum.PARM_REGISTER_NO.getCode());
            result.setMsg(ErrorCodeEnum.PARM_REGISTER_NO.getValue());
            return result;
        }
    ...
    public ActionResult login(ClientInfo clientInfo, String userName, String pwd, String userType,String user_name,
            HttpServletRequest request) throws Exception {
  
        ActionResult result = new ActionResult();
        logger.info("user_name:"+user_name);
        logger.info("all parameters:"+JsonUtil.bean2JsonStr(request.getParameterMap()));
        logger.info("参数为username:{},pwd:{},userType:{}",userName,pwd,userType);
        logger.info("requestparameter arg0",request.getParameter("arg0"));
        logger.info("requestparameter userName",request.getParameter("userName"));
        logger.info("requestparameter user_name",request.getParameter("user_name"));
        if (ValidateUtils.isEmpty(clientInfo) || ValidateUtils.isEmpty(clientInfo.getClient()) || StringUtils.isEmpty(userName) || StringUtils.isEmpty(pwd) || StringUtils.isEmpty(userType)) {
            result.setCode(ErrorCodeEnum.PARM_CHECK_NULL.getCode());
            result.setMsg(ErrorCodeEnum.PARM_CHECK_NULL.getValue());
        ...
        String token = EncryptionUtil.md5Encry(String.valueOf(System.currentTimeMillis()), SaltUtil.createSalt());
        ...
        // token处理
        TokenUtil.setData(clientInfo, userInfo, token);
        ...
        result.addResult("idCard", DesEncryptUtil.des3Encode(userInfo.getIdCard()));
        result.addResult("orgId", userInfo.getOrgId());     
        result.addResult("token", token);
        result.addResult("version", DateInitUtil.getClientVersion());
    ...
    public ActionResult loginNameIsExis(ClientInfo clientInfo, String userName) throws Exception {
        ...
        logger.info("UserController.loginNameIsExist入参:{userName:" + userName + "}");
    ...
    @SystemControllerLog(description = "判断登陆名是否存在")
    public ActionResult loginNameExists(ClientInfo clientInfo, String userName) throws Exception {
        ...
        if (ValidateUtils.isEmpty(userName)) {
            result.setCode(ErrorCodeEnum.PARM_CHECK_NULL.getCode());
            result.setMsg(ErrorCodeEnum.PARM_CHECK_NULL.getValue());
            logger.info("UserController.loginNameExists接口返回:{" + JsonUtil.bean2JsonStr(result) + "}");
            return result;
        }

        logger.info("UserController.loginNameExists入参:{userName:" + userName + "}");
        UserLoginPo po=loginAipService.getLoginInfoByName(userName);
        if (null==po) {
            ...
            result.setSuccess(true);
            result.addResult("success", true);   
            result.addResult("userType", po.getUserType());
        }
    ...
    public ActionResult validateToken(ClientInfo clientInfo, String userId) throws Exception {
        ...
        if (tokenApiService.validateToken(clientInfo.getToken(), userId)) {
            ...
            result.setCode(ErrorCodeEnum.TOKEN_NOTEXISTS.getCode());
            result.setMsg(ErrorCodeEnum.TOKEN_NOTEXISTS.getValue());
            return result;
        }
    ...
    @SystemControllerLog(description = "登录")
    public ActionResult loginNew(ClientInfo clientInfo, String userName, String pwd,
            HttpServletRequest request) throws Exception {
  
        ActionResult result = new ActionResult();
      
        if (ValidateUtils.isEmpty(clientInfo) || ValidateUtils.isEmpty(clientInfo.getClient()) || StringUtils.isEmpty(userName) || StringUtils.isEmpty(pwd)) {
            result.setCode(ErrorCodeEnum.PARM_CHECK_NULL.getCode());
            result.setMsg(ErrorCodeEnum.PARM_CHECK_NULL.getValue());
            logger.info("UserController.loginNew返回结果:{" + JsonUtil.bean2JsonStr(result) + "}");
            return result;
        }

        logger.info("UserController.loginNew入参:{" + JsonUtil.bean2JsonStr(result) + "}");
        ...
        logger.info("UserController.loginNew用户登陆信息:{" + JsonUtil.bean2JsonStr(result) + "}");
        ...
        //企业用户登陆
        if(userInfo.getUserType()==UserInfoConstants.ACCOUNT_PROPERTY_ENTERPRISE_INTEGER){
            result.addResult("companyName", userInfo.getCompanyName());
            result.addResult("buslince", userInfo.getBuslince());
            result.addResult("tel", userInfo.getCorporateTel());
        }
        result.addResult("userId", userInfo.getUserId());
        result.addResult("userName", userInfo.getUserName());      
        result.addResult("userRealName", userInfo.getUserRealName());
        result.addResult("idCard", DesEncryptUtil.des3Encode(userInfo.getIdCard()));      
        result.addResult("userType", userInfo.getUserType());
        result.addResult("token", token);
        result.addResult("version", DateInitUtil.getClientVersion());
        result.setSuccess(true);
    ...
    @SystemControllerLog(description = "虚拟开户注册")
    public ActionResult registerNew(ClientInfo clientInfo, UserInfoVo userInfo) throws Exception {
        ...
        logger.info("UserController.registerNew入参:{" + JsonUtil.bean2JsonStr(userInfo) + "}");
        ...
        if (null != loginPo) {
            ...
            result.setCode(ErrorCodeEnum.REGISTER_EXISTS.getCode());
            result.setMsg(ErrorCodeEnum.REGISTER_EXISTS.getValue());
            logger.info("UserController.registerNew返回结果:{" + JsonUtil.bean2JsonStr(result) + "}");
            return result;
        }
...
//        }
       ...
       // po.setIdCardType(AccountConstants.CERTIFICATE_TYPE_IDENTITY); // 身份证类型
        ...
        /*-----密码处理------*/
        String salt = SaltUtil.createSalt();
        logger.info("UserController.registerNew 盐值:{" + salt+ "}");
        String newPwd = EncryptionUtil.sha256Encry(PwdConstants.REGISTER_NWE_DEF_PWD, salt);
        po.setSalt(salt);
        po.setPwd(newPwd);
        po.setUserType(Integer.parseInt(UserInfoConstants.ACCOUNT_PROPERTY_PERSONAGE));
