...
public class TempController {
    ...
    private void bindCard(CompanyPo companyPo, String userId, Map<String, Integer> mapSuccess) throws Exception {
        ...
        if (null != companyPo.getComBankNum()) {
            ...
            userVo.setIdCardType("X"); // 开户证件类型0：身份证,1: 户口簿，2：护照 x 其他
            userVo.setIdCard(companyPo.getBusLince()); // 证件号(营业执照)
            ...
            /*---------支行信息-----------*/
            cardInfo.setbankbranchname(companyPo.getBankbranchname());  //支行名称
            cardInfo.setBankBranch(companyPo.getBankBranch());  //支行code
            cardInfo.setBankProvince(companyPo.getBankProvince());  //开户行的在省
            cardInfo.setBankCity(companyPo.getBankCity()); //开户行所在市
