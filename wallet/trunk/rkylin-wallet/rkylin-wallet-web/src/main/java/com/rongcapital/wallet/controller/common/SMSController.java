...
public class SMSController {
    ...
    public ActionResult sendSMS(String telNum, String code) throws Exception {
        ...
        if (ulpo == null) {
            logger.info("企业表中取手机号！");
            if (!loginAipService.comTelExists(telNum)) {
    ...
    @ResponseBody
    @RequestMapping("/sendSMSSetTime")
    @SystemControllerLog(description = "发短信，设置超时时间")
    public ActionResult sendSMSSetTime(String telNum, String code, Long time) throws Exception {
        // TODO 是否需要加每个手机号每天最多发多少短信的限制
        ...
        if (!SendSMS.sendSMS(telNum, code, map)) {
            ...
            SmsUtil.setData(telNum, vcode, time);

        }
        logger.info("/sms/sendSMSSetTime.do返回" + JsonUtil.bean2JsonStr(ar));
        ar.setSuccess(true);
        ar.setMsg("发送成功！");
        return ar;
    }
