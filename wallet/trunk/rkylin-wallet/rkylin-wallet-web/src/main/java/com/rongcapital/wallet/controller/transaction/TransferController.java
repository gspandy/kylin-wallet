...
public class TransferController {
    ...
    public ActionResult getWhiteList(ClientInfo clientInfo, String appId, String comUserId) throws Exception {
        ...
        UserWhitePo uwpo = new UserWhitePo();
        uwpo.setApplyId(appId);
        if (!ValidateUtils.isEmpty(comUserId)) {
            uwpo.setUserId(comUserId);
        } else {
            uwpo.setUserId(uipo.getUserId());
        }
        try {
    ...
    /**
     * refType 1.供应商，2.分销商 Discription:
     * 
     * @param clientInfo
     * @param refId
     * @param refName
     ...
     */
    @ResponseBody
    @RequestMapping("/saveWhite")
    @SystemControllerLog(description = "保存白名单信息")
    public ActionResult saveWhite(ClientInfo clientInfo, String refId, String refName, String refType, String appId)
            throws Exception {

        ActionResult ar = new ActionResult();
        if (ValidateUtils.isEmpty(refId) || ValidateUtils.isEmpty(refName) || ValidateUtils.isEmpty(appId)) {
            ar.setSuccess(false);
            ar.setCode(ErrorCodeEnum.PARM_CHECK_NULL.getCode());
        ...
        if (!ValidateUtils.isEmpty(refType)) {
            if (!refType.equals("1") && !refType.equals("2")) {
                ar.setSuccess(false);
                ar.setCode(ErrorCodeEnum.PARM_CHECK_NULL.getCode());
                ar.setMsg("refType值不正确");
                return ar;
            }
        }

        UserWhitePo userWhitePo = new UserWhitePo();
        userWhitePo.setUserId(clientInfo.getUserId());
        userWhitePo.setRefId(refId);
        userWhitePo.setRefName(refName);
        userWhitePo.setRefType(Integer.parseInt(refType));
        userWhitePo.setApplyId(appId);

        try {
            userInfoApiService.saveWhite(userWhitePo);
        } catch (Exception e) {
            logger.error("TransferController.getWhiteList,userInfoApiService.getUserInfoByUserId异常:", e);
            ar.setSuccess(false);
            ar.setCode(ErrorCodeEnum.SYSTEM_ERROR.getCode());
    ...
    @SystemControllerLog(description = "查询白名单信息")
    public ActionResult delWhite(ClientInfo clientInfo, String refId, String appId) throws Exception {

        ActionResult ar = new ActionResult();
        if (ValidateUtils.isEmpty(refId) || ValidateUtils.isEmpty(appId)) {
            ar.setSuccess(false);
            ar.setCode(ErrorCodeEnum.PARM_CHECK_NULL.getCode());
            ar.setMsg(ErrorCodeEnum.PARM_CHECK_NULL.getValue());
        ...
        if (!userInfoApiService.whiteExists(userWhitePo)) {
            ar.setSuccess(false);
            ar.setCode(ErrorCodeEnum.PARM_CHECK_NULL.getCode());
            ar.setMsg("未找到白名单关系");
            return ar;
        ...
        try {
            userInfoApiService.delWhite(userWhitePo);
        } catch (Exception e) {
            logger.error("TransferController.getWhiteList,userInfoApiService.getUserInfoByUserId异常:", e);
            ar.setSuccess(false);
            ar.setCode(ErrorCodeEnum.SYSTEM_ERROR.getCode());
    ...
    @SystemControllerLog(description = "查询交易记录")
    public ActionResult queryOrderWan(ClientInfo clientInfo, String ordertypeid, String orderno, String orderdate,
            String maxresult, String currentpage) throws JsonParseException, JsonMappingException, IOException {
        ActionResult ar = new ActionResult();
